...
const Layout: React.FC<LayoutProps> = ({ children, lang, onLanguageChange, user, onAuthClick, onDashboardClick, onLogout }) => {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center font-bold text-xl overflow-hidden shadow-inner cursor-pointer" onClick={() => window.scrollTo({top: 0, behavior: 'smooth'})}>
            ...
            <div className="flex items-center gap-4 text-white/90 text-sm font-medium">
              {user ? (
                <div className="flex items-center gap-4">
                  <button onClick={onDashboardClick} className="hover:text-white underline decoration-green-400 underline-offset-4">{t.dashboard}</button>
                  <button onClick={onLogout} className="text-red-300 hover:text-red-100">{t.logout}</button>
                </div>
              ) : (
      ...
      <footer className="bg-slate-50 border-t border-slate-200 py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-slate-500 text-sm">
          ...
          <div className="flex justify-center gap-6 mt-4 font-medium">
            ...
            <button className="hover:text-slate-900 transition-colors">Accessibility</button>
          </div>
